<template>
  <div class="chatInterface-container">
    <header class="chatInterface-header">
      ...
      <div class="user-info">
        <img
          ...
          class="user-avatar"
          @click="goToUserProfile(secondUser.id)"
        />
        <div @click="goToUserProfile(secondUser.id)" class="user-username">
          {{ secondUser.username }}
        </div>
      </div>
    </header>
    ...
    <div class="messages-container" ref="messagesContainer" @scroll="handleScroll" tabindex="0">
      <div
        ...
        :key="message.id"
        :class="[ 'message-item', message.senderId === secondUser.id ? 'left-message' : 'right-message' ]"
      ...
      >
        ...
        <div v-if="message.senderId !== secondUser.id" class="message-options">
...
import {
  createMessage,
  deleteMessage,
  getAllMessages, getMessageFile,
  readMessage, updateMessageContent
} from "@/js/service/messageService";
import router from "@/js/config/router";
import {getPrivateChat} from "@/js/service/privateChatService";
...
export default {
  ...
  methods: {
    ...
    async fetchChatData() {
      try {
        const chatData = await getPrivateChat(this.chatId);

        this.secondUser = chatData.secondUser;
        this.privateChat = chatData.privateChat;
        this.lastMessage = chatData.lastMessage;
        this.unreadMessage = chatData.unreadMessage;

        if (chatData.secondUser.userProfilePhoto?.id) {
          this.userPhoto = await getUserPhoto(chatData.secondUser.userProfilePhoto.id);
        } else {
          this.userPhoto = null;
        }
      } catch (error) {
    ...
    handleVisibleMessages(entries) {
      entries.forEach(async (entry) => {
        if (entry.isIntersecting) {
          ...
          if (message && message.senderId === this.secondUser.id && !message.isRead) {
