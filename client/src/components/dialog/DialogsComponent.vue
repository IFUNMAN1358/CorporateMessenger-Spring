  ...
  <div class="dialogs-container" @click="handleOutsideClick">
    ...
    <div class="dialogs-list">
      ...
      <div
        ...
        :key="chat.privateChat.id"
        class="dialog-item"
        @click="goToPrivateChat(chat.privateChat.id)"
      >
        <img
          :src="chat.secondUser.userProfilePhoto || defaultAvatar"
          alt="avatar"
          class="chat-avatar"
        />
        <div class="chat-info">
          <p class="chat-user-name">{{ chat.secondUser.username }}</p>
          <p class="chat-last-message">
            {{ chat.lastMessage?.content || "Нет сообщений" }}
          </p>
        </div>
        <div class="chat-meta">
          <p class="chat-time">{{ formatTime(chat.lastMessage?.createdAt) }}</p>
          <p v-if="chat.unreadMessage.unreadCount > 0" class="unread-count">
      ...
      <div
        ...
        class="dialog-item"
        @click="goToGroupChat(chat.groupChat.id)"
      >
        <img
          :src="chat.groupChat.filePath || groupChatDefaultAvatar"
          alt="avatar"
          class="chat-avatar"
        />
        <div class="chat-info">
          <p class="chat-user-name">{{ chat.groupChat.name }}</p>
          <p class="chat-last-message">
            <template v-if="chat.lastMessage">
              <strong>{{ chat.lastMessageSender.firstName }}: </strong>
      ...
      try {
        ...
        this.privateChats = await Promise.all(
          chats.map(async (chat) => {
            if (chat.secondUser.userProfilePhoto?.id) {
              chat.secondUser.userProfilePhoto = await getUserPhoto(chat.secondUser.userProfilePhoto.id);
            } else {
              chat.secondUser.userProfilePhoto = null;
            }
            return chat;
          })
        );
    ...
    async fetchGroupChats() {
      try {
        ...
        this.groupChats = await Promise.all(
          chats.map(async (chat) => {
            chat.groupChat.filePath = chat.groupChat.filePath
              ? await getGroupChatPhoto(chat.groupChat.id)
              : groupChatDefaultAvatar;

            if (chat.lastMessage) {
              chat.lastMessageSender = {
                ...chat.lastMessageSender,
                userFirstName: chat.lastMessageSender.firstName,
              };
            }
            return chat;
          })
